...
type MyStyleMap = Map<PropertyKey, string>;
...
class SmootherCornersPainter implements Paint {
  ...
  superellipse(a: number, b: number, nX = 4, nY: number) {
  ...
  paint(ctx: PaintRenderingContext2D, geom: PaintSize, properties: MyStyleMap) {
    const rawParameters = properties.get("--smooth-corners") ?? '5'
    const [nX, nY] = rawParameters
      .toString()
      .replace(/ /g, "")
      .split(",");
